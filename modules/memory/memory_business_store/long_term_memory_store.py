...
from utils.monitor_utils.metrics import memory_write_total, memory_write_duration_seconds
...
class LongTermMemoryStore(BaseMemoryStore):
    def __init__(self, vector_store: BaseVectorStore, config: MemorySystemConfig,cursor_manager: CursorManager):
        """
    ...
    def add_memory(
        ...
        try:
            ...
            memory_write_duration_seconds.labels(user_id).observe(time.monotonic() - start_time)
            ...
            if memory_type == MemoryType.USER_PROFILE:
                ...
                if model.source in [MemorySource.LOAN_ADVISOR.value,MemorySource.AFTER_LOAN.value,MemorySource.RISK_ASSESSMENT.value]:
                    self._invalidate_sub_recent_interactions(user_id)
        ...
        # output metrics
        memory_write_total.labels(type=memory_type.value, user=user_id).inc()
        return memory_id
    ...
    def get_sub_recent_interactions(self,user_id: str, limit: int = 5) -> Optional[List[Dict[str, Any]]]:
        logger.info("Fetching recent interactions for user=%s, limit=%d", user_id, limit)
        # build query_utils condition
        where = Query(conditions=[
            Condition(field=MemoryFields.USER_ID, op="==", value=user_id),
            Condition(field=MemoryFields.STATUS, op="==", value=MemoryStatus.ACTIVE.value),
            Condition(field=CommonFields.SOURCE, op="in",value=[AgentName.AFTER_LOAN.value, AgentName.LOAN_ADVISOR.value, AgentName.RISK_ASSESSMENT.value])
        ])
    ...
    def get_profile_summary(self, user_id: str, max_chars: int = 500) -> Optional[str]:
        ...
        try:
            ...
            # sensitive field desensitization regex
            def mask_content(key, content):
                if not isinstance(content, str):
                    return content
                if key in self.config.sensitive_keys:
                    if len(content) > 4:
    ...
    def _invalidate_profile_summary(self, user_id: str) -> None:
        try:
            ...
            if cache:
                ...
                cache.invalidate("get_all_user_profile_memories", user_id)
    ...
    def _invalidate_sub_recent_interactions(self, user_id: str) -> None:
        try:
            cache = get_registry_manager(CacheNamespace.RECENT_INTERACTION)
            if cache:
                cache.invalidate("get_sub_recent_interactions", user_id)
                logger.info("Recent sub interactions cache invalidated for user=%s", user_id)
        except Exception as e:
            logger.warning("Failed to invalidate sub recent interactions cache for user=%s: %s", user_id, e)
