...
def get_config() -> ConfigRegistry:
    ...
    path7 = Path(PROJECT_ROOT / "config" / "rules" / "loan_advisor.yaml")
    path8 = Path(PROJECT_ROOT / "config" / "rules" / "risk_assessment.yaml")
    path9 = Path(PROJECT_ROOT / "config" / "rules" / "after_loan.yaml")
    ...
    registry.register_model(RegistryModules.LOAN_ADVISOR, LoanAdvisorConfig, path7)
    registry.register_model(RegistryModules.RISK_ASSESSMENT, RiskAssessmentConfig, path8)
    registry.register_model(RegistryModules.AFTER_LOAN, AfterLoanConfig, path9)
...
if __name__ == '__main__':
    ...
    print(registry.get_config(RegistryModules.LOAN_ADVISOR))
    print(registry.get_config(RegistryModules.RISK_ASSESSMENT))
    print(registry.get_config(RegistryModules.AFTER_LOAN))
