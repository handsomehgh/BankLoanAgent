...
class MemorySystemConfig(BaseModel):
    ...
    #confidec threshold
    high_conf_threshold: float = Field(default=0.7)
    medium_conf_threshold: float = Field(default=0.5)

    #sensitive info
    sensitive_keys: List[str] = Field(default_factory=list)

    #asynchronous extraction flag
    async_log_enabled: bool = True

    #sub summary
    sub_summary_enabled: bool = True
    sub_summary_threshold: int = 6
