...
#general fields(shared by all collections)
COMMON_FIELDS = [
    FieldSchema(name="id",dtype=DataType.VARCHAR,is_primary=True,max_length=100,description="unique id for memory"),
    FieldSchema(name="text",dtype=DataType.VARCHAR,is_primary=True,max_length=65535,description="memory content"),
    FieldSchema(name="dense_vector",dtype=DataType.FLOAT_VECTOR,dim=1024,description="dense semantic vector"),
    FieldSchema(name="sparse_vector",dtype=DataType.SPARSE_FLOAT_VECTOR,description="sparse keyword vector(BM25)"),
    FieldSchema(name="user_id",dtype=DataType.VARCHAR,max_len=64,description="unique id for user"),
    FieldSchema(name="status",dtype=DataType.VARCHAR,max_len=20,description="status(active,superseded,forgotten)"),
    FieldSchema(name="confidence",dtype=DataType.FLOAT,description="confidence"),
    FieldSchema(name="metadata",dtype=DataType.JSON,description="extended metadata"),
    FieldSchema(name="created_at", dtype=DataType.VARCHAR, max_length=30, description="create time"),
    FieldSchema(name="permanent", dtype=DataType.BOOL, description="whether permanent save"),
    FieldSchema(name="last_accessed_at", dtype=DataType.VARCHAR, max_length=30, description="last access time"),
]

#user profile specific fields
USER_PROFILE_EXTRA_FIELDS = [
    FieldSchema(name="entity_key",dtype=DataType.VARCHAR,max_len=64,description="entity type(income,occupation...)"),
    FieldSchema(name="evidence_type",dtype=DataType.VARCHAR,max_len=32,description="the type of evidence"),
    FieldSchema(name="effective_date",dtype=DataType.VARCHAR,max_len=30,description="effective date"),
...
#interaciton log specific fields
INTERACTION_LOG_EXTRA_FIELDS = [
    FieldSchema(name="session_id",dtype=DataType.VARCHAR,max_len=64,description="session id"),
    FieldSchema(name="timestamp",dtype=DataType.VARCHAR,max_len=30,description="interaction time"),
    FieldSchema(name="event_type",dtype=DataType.VARCHAR,max_len=32,description="event type"),
    FieldSchema(name="sentiment",dtype=DataType.VARCHAR,max_len=20,description="sentiment label"),
    FieldSchema(name="key_entities", dtype=DataType.ARRAY, element_type=DataType.VARCHAR, max_capacity=20),
    FieldSchema(name="source", dtype=DataType.VARCHAR, max_length=32),
]

#compliance rule specific fields
COMPLIANCE_RULE_EXTRA_FIELDS = [
    FieldSchema(name="rule_id",dtype=DataType.VARCHAR,max_len=32,description="unique id for rule"),
    FieldSchema(name="rule_name",dtype=DataType.VARCHAR,max_len=100,description="rule name"),
    FieldSchema(name="rule_type",dtype=DataType.VARCHAR,max_len=32,description="the type of rule(forbidden_phrase,sensitive_keyword)"),
    FieldSchema(name="pattern",dtype=DataType.VARCHAR,max_len=1000,description="regular expression"),
    FieldSchema(name="severity",dtype=DataType.VARCHAR,max_len=20,description="severity"),
    FieldSchema(name="priority",dtype=DataType.INT64,description="priority(the smaller the number,the higher the priority)"),
    FieldSchema(name="action",dtype=DataType.VARCHAR,max_len=20,description="action(block,warn,append)"),
    FieldSchema(name="template", dtype=DataType.VARCHAR, max_length=500, description="template text for the APPEND action"),
    FieldSchema(name="source", dtype=DataType.VARCHAR, max_length=200, description="source of rules (e.g., 'Financial Regulatory Authority')"),
    FieldSchema(name="effective_to",dtype=DataType.VARCHAR,max_len=30,description="invalidation Time"),
    FieldSchema(name="effective_from",dtype=DataType.VARCHAR,max_len=30,description="effective time"),
]
...
SCALAR_INDEX_FIELDS = {
    MemoryType.USER_PROFILE.value: ["user_id", "entity_key", "status"],
    MemoryType.INTERACTION_LOG.value: ["user_id", "session_id", "timestamp", "status"],
    MemoryType.COMPLIANCE_RULE.value: ["rule_id", "severity", "status"]
}
...
def create_index(col: Collection,dense_field: str = "dense_vector",sparse_field: str = "sparse_vector"):
    ...
    if not col.has_index(index_name=sparse_idx_name):
        col.create_index(
            ...
            index_name=f"{sparse_field}_idx"
        )
    else:
