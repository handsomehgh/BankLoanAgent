...
def import_profiles(jsonl_path: str,store: LongTermMemoryStore):
    """导入用户画像测试数据"""
        ...
        for line_num, line in enumerate(f, 1):
            ...
            try:
                memory_id = store.add_memory(
                    ...
                    entity_key=ProfileEntityKey(data["entity_key"]),
                    metadata=metadata
                )
...
def verify_import(store: LongTermMemoryStore):
    ...
    for user_id in ["test_user_001", "test_user_002"]:
        print(f"\n👤 用户: {user_id}")
        memories = store.get_all_user_profile_memories(user_id, status=MemoryStatus.ACTIVE)
        if memories:
            for mem in memories:
                meta = mem["metadata"]
